...
class ClearCacheAdmin extends \ProcessWire\Process{
    ...
    public function ___execute(){
        ...
        $submit->attr("value", \ProcessWire\__("Clear now"));
        ...
        if($this->wire("cache")) {
            ...
            if(count($expiringCaches)){
                ...
                $tableMarkup->label = \ProcessWire\__('Clear the \ProcessWire\wirecache($cache)');
                $tableMarkup->notes = \ProcessWire\__("As found in the wire cache DB table except those that never expire.");
                $toggleBtn = "<label>";
                $toggleBtn .= "<input class='toggle_all' data-target='$wireCacheID' type='checkbox'> ";
                $toggleBtn .= \ProcessWire\__("toggle all");
                $toggleBtn .= "</label><br/>";
                $tableMarkup->value = "";
                ...
                $tableMarkup = $modules->InputfieldMarkup;
                $tableMarkup->label = \ProcessWire\__('Clear the \ProcessWire\wirecache($cache)');
                $tableMarkup->value = \ProcessWire\__("No Wire Caches found");
                $form->add($tableMarkup);
                $out .= $form->render();
        ...
        if(count($cacheDirs)){
            ...
            $f->label = \ProcessWire\__("Clear other files or directories?");
            $f->notes = \ProcessWire\__("Other files and directories as found in site/assets/cache/. They will get deleted recursively.");
    ...
    public function ___executeClearMarkupCache(){
        ...
        $this->message(sprintf(\ProcessWire\__("Cleared %d MarkupCache files and dirs."), $numFiles));
        $modules->session->redirect($this->wire("page")->url);
    }
    ...
    public function ___executeClearWireCache(){
        ...
        if(count($caches)){
            foreach($caches as $name => $value){
                ...
                $this->message(sprintf(\ProcessWire\__("Cleared %s WireCache."), $name));
            }
    ...
    public function ___executeClearCacheDirs(){
        ...
        if(count($dirs)){
            foreach($dirs as $filename => $v){
                ...
                if(is_dir($path)) {
                    \ProcessWire\wirermdir($path, true);
                    $this->message(sprintf(\ProcessWire\__("Cleared directory %s"), $path));
                } else {
                    @unlink($path);
                    $this->message(sprintf(\ProcessWire\__("Cleared file %s"), $path));
                }
            }
        }
    ...
    public function ___executeNavJson(array $options = array()){
        ...
        $data["pagecache"] = array(
            "id" => "clearpagecache",
            "name" => sprintf(\ProcessWire\__("Clear Page Render Disk Cache (%d Pages)"), $numPages),
            "icon" => "bomb",
            );
        ...
        if($modules->isInstalled("MarkupCache")){
             $data["markupcache"] = array(
                "id" => "clearmarkupcache",
                "name" => \ProcessWire\__("Clear Markup Cache"),
                "icon" => "bomb"
                );
        }
        ...
        if(count($cacheDirs)){
            foreach($cacheDirs as $filename => $cd){
                $data[$filename] = array(
                    ...
                    "name" => \ProcessWire\__("Clear") . " cache/" . $filename . " ({$cd['type']})",
        ...
        if(count($expiringCaches)){
            foreach($expiringCaches as $cache){
                $data["wirecache_" . $cache['name']] = array(
                    ...
                    "name" => sprintf(\ProcessWire\__("Clear %s (WireCache)"), $cache['name']),
                    "icon" => "database",
                    );
            }
        }
    ...
    protected function getCacheNumPages(){
        $path = $this->wire('config')->paths->cache . \ProcessWire\PageRender::cacheDirName . '/';
        $numPages = 0;
        $dir = null;
